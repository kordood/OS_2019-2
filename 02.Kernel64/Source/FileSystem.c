...
static BYTE gs_vbTempDir[ FILESYSTEM_SECTORSPERCLUSTER * 512 ];
static BYTE gs_vbTempChange[ FILESYSTEM_SECTORSPERCLUSTER * 512 ];

extern DWORD currentDirectoryClusterIndex;

char prompt_path[100];
//DWORD currentDirectory->dwStartClusterIndex = 0;
DWORD gs_dwTmpCurrentClusterIndex = 0;
...
void kChangeDirectory (const char* pcParameterBuffer){
	...
	iLength = kGetNextParameter(&stList, vcPath);
	vcPath[iLength] = '\0';

	// cd '@!@#$'
	if(iLength > (FILESYSTEM_MAXFILENAMELENGTH - 1)){
		kPrintf("Too Long Path\n");
		return ;
	}

	(&stList)->pcBuffer = vcPath;
	(&stList)->iLength = kStrLen( vcPath );
	(&stList)->iCurrentPosition = 0;


	if(iLength == 0 || vcPath[0] == '/'){
		if(kReadCluster(0, gs_vbTempChange) == FALSE){
			kPrintf("error!!\n");
			return;
		}			

		currentDirectory = (DIRECTORYENTRY*)gs_vbTempChange;
		currentDirectoryClusterIndex = currentDirectory->dwStartClusterIndex;
		idx = 0;
	}

	if(vcPath[0] == '/')
	{
		int tmpLen = (&stList)->iLength;
	...
	while(kGetNextPath(&stList, vcPath) !=0 ){
		int st = 0;
	// directory
		if(vcPath[0] == '.' && vcPath[1] == '.'){
			if(kReadCluster(currentDirectory[1].dwStartClusterIndex, gs_vbTempChange) == FALSE){
				kPrintf("error!!\n");
				return;
			}
		...
		for(int j = 0; j < FILESYSTEM_MAXDIRECTORYENTRYCOUNT; j++) {
			...
			if(kMemCmp (currentDirectory[j].vcFileName, vcPath, kStrLen(vcPath)) == 0 &&
				currentDirectory[j].bFileType == FILESYSTEM_TYPE_DIRECTORY){
				st = 1;
				idx++;
				kMemCpy(pvPath[idx], currentDirectory[j].vcFileName, sizeof(currentDirectory[j].vcFileName));
				int len = kStrLen(currentDirectory[j].vcFileName);
				pvPath[idx][len] = '\0';
				if(kReadCluster(currentDirectory[j].dwStartClusterIndex, gs_vbTempChange) == FALSE){
					kPrintf("error!!\n");
					return;
				}
				currentDirectory = (DIRECTORYENTRY*)gs_vbTempChange;
...
int kGetNextPath(PARAMETERLIST* pstList, char* pcPath){
	...
	kMemCpy(pcPath, pstList->pcBuffer + pstList->iCurrentPosition, i);
...
BOOL kInitializeFileSystem( void )
{
	...
	currentDirectory = (DIRECTORYENTRY*)gs_vbTempBuffer;
...
static int kFindFreeDirectoryEntry( DWORD dwClusterIndex )
{
	...
	// 디렉터리를 읽음
	if( kReadCluster( dwClusterIndex, gs_vbTempBuffer ) == FALSE )
	{
		return -1;
	}
	...
	{
		if( pstEntry[ i ].dwStartClusterIndex != 0 || (pstEntry[ i ].bFileType == FILESYSTEM_TYPE_DIRECTORY && pstEntry[ i ].dwStartClusterIndex == 0))
		{
		}
		else{
			return i;
...
static BOOL kSetDirectoryEntryData( int iIndex, DIRECTORYENTRY* pstEntry, DWORD dwClusterIndex )
{
	...
	// 디렉터리를 읽음
	if( kReadCluster( dwClusterIndex, gs_vbTempDir ) == FALSE )
	{
		return FALSE;
	}    

	// 디렉터리에 있는 해당 데이터를 갱신
	pstRootEntry = ( DIRECTORYENTRY* ) gs_vbTempDir;
	kMemCpy( pstRootEntry + iIndex, pstEntry, sizeof( DIRECTORYENTRY ) );

	// 디렉터리에 씀
	if( kWriteCluster( dwClusterIndex, gs_vbTempDir ) == FALSE )
	{
		return FALSE;
	}    
...
static BOOL kGetDirectoryEntryData( int iIndex, DIRECTORYENTRY* pstEntry )
{
	...
	// 루트 디렉터리를 읽음
	if( kReadCluster( currentDirectory->dwStartClusterIndex, gs_vbTempBuffer ) == FALSE )
	{
		return FALSE;
	}    
...
{
	currentDirectory->dwStartClusterIndex = currentDirectoryClusterIndex;
}

void kSetTmpClusterIndex(DWORD currentDirectoryClusterIndex)
{
	gs_dwTmpCurrentClusterIndex = currentDirectoryClusterIndex;
}
/**
 *  해당 디렉토리의 엔트리 배열 리턴
 */
	...
	int* piDirectoryEntryIndex )
{
	...
	BYTE bDayofMonth, bMonth, bDayofWeek;
	WORD wYear;
	...
	// 빈 디렉터리 엔트리를 검색
	*piDirectoryEntryIndex = kFindFreeDirectoryEntry( currentDirectory->dwStartClusterIndex );
	...
	pstEntry->bFileType = FILESYSTEM_TYPE_FILE;
	pstEntry->stFileCreationTime.bSecond = bSecond;
	pstEntry->stFileCreationTime.bMinute = bMinute;
	pstEntry->stFileCreationTime.bHour = bHour;
	pstEntry->stFileCreationTime.bDayofMonth = bDayofMonth;
	pstEntry->stFileCreationTime.bMonth = bMonth;
	pstEntry->stFileCreationTime.wYear = wYear;


	// 디렉터리 엔트리를 등록
	if( kSetDirectoryEntryData( *piDirectoryEntryIndex, pstEntry, currentDirectory->dwStartClusterIndex ) == FALSE )
	{
		// 실패할 경우 할당 받은 클러스터를 반환해야 함
		kSetClusterLinkData( dwCluster, FILESYSTEM_FREECLUSTER );
		return FALSE;
	...
	int* piDirectoryEntryIndex, DWORD dwParentClusterIndex)
{
	...
	// 빈 디렉터리 엔트리를 검색
	*piDirectoryEntryIndex = kFindFreeDirectoryEntry( dwParentClusterIndex );
	if( *piDirectoryEntryIndex == -1 )
	{
		// 실패할 경우 할당 받은 클러스터를 반환해야 함
		kSetClusterLinkData( dwCluster, FILESYSTEM_FREECLUSTER );
	...
	// 디렉터리 엔트리를 설정
	kMemCpy( pstEntry->vcFileName, pcDirName, kStrLen( pcDirName ) + 1 );
	pstEntry->dwStartClusterIndex = dwCluster;
	pstEntry->dwFileSize = 0x1000;

	kReadRTCDate(&wYear, &bMonth, &bDayofMonth, &bDayofWeek);
	kReadRTCTime(&bHour, &bMinute, &bSecond);

	pstEntry->bFileType = FILESYSTEM_TYPE_DIRECTORY;
	pstEntry->stFileCreationTime.bSecond = bSecond;
	pstEntry->stFileCreationTime.bMinute = bMinute;
	pstEntry->stFileCreationTime.bHour = bHour;
	pstEntry->stFileCreationTime.bDayofMonth = bDayofMonth;
	...
	// 디렉터리 엔트리를 등록
	if( kSetDirectoryEntryData( *piDirectoryEntryIndex, pstEntry, dwParentClusterIndex ) == FALSE )
	{
		// 실패할 경우 할당 받은 클러스터를 반환해야 함
		kSetClusterLinkData( dwCluster, FILESYSTEM_FREECLUSTER );
		return FALSE;
	...
	kSetDotInDirectory( dwCluster, dwParentClusterIndex );

	return TRUE;
}

/** 
 * 루트 디렉토리의 dot, dot dot 생성
 */
void kSetDotInRootDirectory(){
    ...
    stEntry.dwStartClusterIndex = 0;                // root cluster
    stEntry.dwFileSize = 0x1000;
    stEntry.bFileType = FILESYSTEM_TYPE_DIRECTORY;
    
    kReadRTCDate(&wYear, &bMonth, &bDayofMonth, &bDayofWeek);
    kReadRTCTime(&bHour, &bMinute, &bSecond);

    stEntry.stFileCreationTime.bSecond = bSecond;
    stEntry.stFileCreationTime.bMinute = bMinute;
    stEntry.stFileCreationTime.bHour = bHour;
    stEntry.stFileCreationTime.bDayofMonth = bDayofMonth;
    stEntry.stFileCreationTime.bMonth = bMonth;
    stEntry.stFileCreationTime.wYear = wYear;
    
    // 디렉터리 엔트리를 등록
    if( kSetDirectoryEntryData( iDirectoryEntryOffset, &stEntry, 0 ) == FALSE )
    {
 ...
 */
void kSetDotInDirectory( DWORD dwCurrentClusterIndex, DWORD dwParentClusterIndex ){
	DIRECTORYENTRY stEntry;
	DIRECTORYENTRY *pstEntryBuffer;
	DIR* pstDirectory;
	int iDirectoryEntryOffset = 0;
	BYTE bSecond, bMinute, bHour;
	BYTE bDayofMonth, bMonth, bDayofWeek;
    ...
    // 디렉터리 엔트리를 설정
	kMemCpy( stEntry.vcFileName, ".", 2 );
	stEntry.dwStartClusterIndex = dwCurrentClusterIndex;
	stEntry.dwFileSize = 0x1000;
	stEntry.bFileType = FILESYSTEM_TYPE_DIRECTORY;
    ...
    // 디렉터리 엔트리를 등록
	if( kSetDirectoryEntryData( iDirectoryEntryOffset, &stEntry, dwCurrentClusterIndex ) == FALSE )
	{  
		return FALSE;
	}
    ...
    // 디렉터리 엔트리를 설정
	kMemCpy( stEntry.vcFileName, "..", 3 );
	stEntry.dwStartClusterIndex = dwParentClusterIndex;
	stEntry.dwFileSize = 0x1000;
	stEntry.bFileType = FILESYSTEM_TYPE_DIRECTORY;

	pstDirectory = kOpenDirectoryHandler(dwParentClusterIndex);
	pstEntryBuffer = readdir( pstDirectory );

	stEntry.stFileCreationTime.bSecond = pstEntryBuffer->stFileCreationTime.bSecond;
	stEntry.stFileCreationTime.bMinute = pstEntryBuffer->stFileCreationTime.bMinute;
	stEntry.stFileCreationTime.bHour = pstEntryBuffer->stFileCreationTime.bHour;
	stEntry.stFileCreationTime.bDayofMonth = pstEntryBuffer->stFileCreationTime.bDayofMonth;
	stEntry.stFileCreationTime.bMonth = pstEntryBuffer->stFileCreationTime.bMonth;
	stEntry.stFileCreationTime.wYear = pstEntryBuffer->stFileCreationTime.wYear;
...
FILE* kOpenFile( const char* pcFileName, const char* pcMode )
{
	...
	//==========================================================================
	iDirectoryEntryOffset = kFindDirectoryEntry( pcFileName, &stEntry, currentDirectory->dwStartClusterIndex );
	...
	else if( pcMode[ 0 ] == 'w' )
	{
		...
		if( kSetDirectoryEntryData( iDirectoryEntryOffset, &stEntry, currentDirectory->dwStartClusterIndex ) == FALSE )
		{
			// 동기화
			kUnlock( &( gs_stFileSystemManager.stMutex ) );
			return NULL;
...
static BOOL kUpdateDirectoryEntry( FILEHANDLE* pstFileHandle )
{
	...
	// 변경된 디렉터리 엔트리를 설정
	if( kSetDirectoryEntryData( pstFileHandle->iDirectoryEntryOffset, &stEntry, currentDirectory->dwStartClusterIndex )
		== FALSE )
	{
		return FALSE;
	}
 ...
 *  파일을 삭제
 */
int kRemoveDirectory( const char* pcDirPath )
{
	DIRECTORYENTRY stEntry;
	int iDirectoryEntryOffset;
	int iDirNameLength;
	DWORD dwCluster, dwObjectCluster;
	char pcDirName[100];

	kMemCpy( pcDirName, pcDirPath, kStrLen( pcDirPath ) + 1);

	kPrintf("path: %s\n", pcDirName);
	dwObjectCluster = kGetObjectCluster( pcDirName );						// cluster of making dir's parent
	kPrintf("path: %s cluster: %d\n", pcDirName, dwObjectCluster);
	if(dwObjectCluster == -1){
		return NULL;
	}
	...
	// 파일이 존재하는가 확인
	iDirectoryEntryOffset = kFindDirectoryEntry( pcDirName, &stEntry, dwObjectCluster );
	if( iDirectoryEntryOffset == -1 ) 
	{ 
		// 동기화
		kUnlock( &( gs_stFileSystemManager.stMutex ) );
	...
	// 디렉터리 엔트리를 빈 것으로 설정
	kMemSet( &stEntry, 0, sizeof( stEntry ) );
	if( kSetDirectoryEntryData( iDirectoryEntryOffset, &stEntry, dwObjectCluster ) == FALSE )
	{
		// 동기화
		kUnlock( &( gs_stFileSystemManager.stMutex ) );
		return -1;
...
// 디렉터리가 존재하지 않을 때만 생성
//==========================================================================
iDirectoryEntryOffset = kFindDirectoryEntry( pcDirName, &stEntry );
if( iDirectoryEntryOffset == -1 )
{
dwCluster = kCreateDir( pcDirName, &stEntry, &iDirectoryEntryOffset);
if( dwCluster == FALSE )
{
// 동기화
kUnlock( &( gs_stFileSystemManager.stMutex ) );
...
//==========================================================================
// 디렉터리 핸들을 할당 받아 데이터를 설정한 후 반환
//==========================================================================
// 디렉터리 핸들을 할당 받아 데이터 설정
pstDir = kAllocateFileDirectoryHandle();
if( pstDir == NULL )
{
// 동기화
kUnlock( &( gs_stFileSystemManager.stMutex ) );
...
// 디렉터리를 저장할 버퍼를 할당
pstDirBuffer = ( DIRECTORYENTRY* ) kAllocateMemory( FILESYSTEM_CLUSTERSIZE );
if( pstDir == NULL )
{
// 실패하면 핸들을 반환해야 함
kFreeFileDirectoryHandle( pstDir );
// 동기화
kUnlock( &( gs_stFileSystemManager.stMutex ) );
return NULL;
}
...
// 생성한 디렉터리를 읽음
if( kReadCluster( dwCluster, ( BYTE* ) pstDirBuffer ) == FALSE )
{
// 실패하면 핸들과 메모리를 모두 반환해야 함
kFreeFileDirectoryHandle( pstDir );
kFreeMemory( pstDirBuffer );
...
pstDir->bType = FILESYSTEM_TYPE_DIRECTORY;
pstDir->stDirectoryHandle.iCurrentOffset = 0;
pstDir->stDirectoryHandle.pstDirectoryBuffer = pstDirBuffer;

kFindDirectoryEntry( ".", &stEntry );
iDirectoryEntryOffset = 0;
kCreateDir( ".", pstDirBuffer, &iDirectoryEntryOffset );
iDirectoryEntryOffset = 1;
kCreateDir( "..", pstDirBuffer, &iDirectoryEntryOffset );

// 생성한 디렉터리를 읽음
if( kWriteCluster( dwCluster, ( BYTE* ) pstDirBuffer ) == FALSE )
{
	// 실패하면 핸들과 메모리를 모두 반환해야 함
	kFreeFileDirectoryHandle( pstDir );
	kFreeMemory( pstDirBuffer );
...
DIR* kOpenDirectoryHandler( DWORD dwCluster )
{
	...
	// 현재 디렉터리를 읽음
	if( kReadCluster( dwCluster, ( BYTE* ) pstDirectoryBuffer ) == FALSE )
	{
		// 실패하면 핸들과 메모리를 모두 반환해야 함
		kFreeFileDirectoryHandle( pstDirectory );
...
// 디렉터리인지 검사
BOOL isDirectory( const char* pcDirPath )
{
	DIRECTORYENTRY stEntry;
	int iDirectoryEntryOffset;
	DWORD dwObjectCluster;
	char pcDirName[100];
	...
	if( iDirectoryEntryOffset == -1 && pcDirName[0] != "/")
	{
		// 동기화
		kUnlock( &( gs_stFileSystemManager.stMutex ) );
		return FALSE;
	...
	if( stEntry.bFileType != FILESYSTEM_TYPE_DIRECTORY ){
		return FALSE;
	}
	return TRUE;
}
...
// 빈 디렉터리인지 검사
BOOL isEmptyDirectory( const char* pcDirPath )
{
	DIR* pstDirectory;
	DIRECTORYENTRY stEntry;
	DIRECTORYENTRY* pstEntry;
	int iTotalCount, iDirectoryEntryOffset;
	DWORD dwObjectCluster;
	char pcDirName[100];
	...
	// 현재 디렉터리를 읽음
	pstDirectory = opendirhand( stEntry.dwStartClusterIndex );
	...
	while( 1 )
	{
		...
		if( kMemCmp( ".\0", pstEntry->vcFileName, 2) == 0 || kMemCmp( "..\0", pstEntry->vcFileName, 3) == 0) {
			continue;
		}
		iTotalCount++;
	}
...
DIR* kOpenDirectory( const char* pcDirPath)
{
	...
	if( iDirectoryEntryOffset == -1 && pcDirName[0] != "/")
	{
		kPrintf("\tdirectory creating ... at %d", dwObjectCluster);
		if( kCreateDirectory( pcDirName, &stEntry, &iDirectoryEntryOffset, dwObjectCluster) == FALSE )
		{
			// 동기화
			kUnlock( &( gs_stFileSystemManager.stMutex ) );
			return NULL;
	...
	// Update entry of currentDirectory
	if(kReadCluster(currentDirectory->dwStartClusterIndex, gs_vbTempChange) == FALSE){
		kPrintf("error!!\n");
		return;
	}
	currentDirectory = (DIRECTORYENTRY*)gs_vbTempChange;
	...
	// 생성된 디렉터리의 부모를 읽음
	if( kReadCluster( dwObjectCluster, ( BYTE* ) pstDirBuffer ) == FALSE )
	{
		// 실패하면 핸들과 메모리를 모두 반환해야 함
		kFreeFileDirectoryHandle( pstDir );
		kFreeMemory( pstDirBuffer );
...
DWORD kGetObjectCluster( char* pcDirName ){
	DIRECTORYENTRY* pstDirectory;
	char vcPath[100], vcTmp[100];
	int iLength, iFlag = 0;
	int iIndex = 0, iIndexTmp = 0, iIndexEntry;
	DWORD dwCluster = 0;

	iLength = kStrLen(pcDirName);
	kMemCpy(vcPath, pcDirName, iLength + 1);

	if(vcPath[0] == '/') {										// Absolute path
		vcTmp[0] = '/';
		iIndex += 1;

		if(kReadCluster(dwCluster, pstDirectory) == FALSE){
			kPrintf("error1!!\n");
			return -1;
		}
		iFlag = 1;
		...
		if(!kMemCmp("..", vcPath, 2)){							// parent
			kMemCpy(vcTmp, vcPath, 2);
			iIndex += 2;

			//	=== read cluster of parent ===
			if(kReadCluster(currentDirectory->dwStartClusterIndex, pstDirectory) == FALSE){
				kPrintf("error2!!\n");
				return -1;
			}

			dwCluster = pstDirectory[ 1 ].dwStartClusterIndex;
		}
		else if(vcPath[0] == "."){								// self
			vcTmp[0] = vcPath[0];
			iIndex += 1;	

			//	=== read cluster of current ===
			if(kReadCluster(currentDirectory->dwStartClusterIndex, pstDirectory) == FALSE){
				kPrintf("error2!!\n");
				return -1;
			}

			dwCluster = pstDirectory[ 0 ].dwStartClusterIndex;
		}
		else{													// ex) input: foo (i.e. makedir foo)
			return currentDirectory->dwStartClusterIndex;
		}
	...
	kMemSet( vcTmp, 0, 100 );
	kMemSet( pcDirName, 0, 100 );

	while(vcPath[iIndex] != '\0'){
		if(vcPath[iIndex] == '/') {

			if( vcPath[iIndex] == '.' && vcPath[iIndex + 1] == '.'){
				kMemCpy(vcTmp, vcPath, 2);
				iIndex += 2;
				if(kReadCluster(dwCluster, pstDirectory) == FALSE){
					kPrintf("error5-1!!\n");
				...
				if(iFlag != 1){
					iIndexEntry = kFindDirectoryEntry( vcTmp, pstDirectory, dwCluster );
					if( iIndexEntry == -1 ){
						kPrintf("error5!! %s\n", vcTmp);
						return -1;
					}
					dwCluster = pstDirectory[ iIndexEntry ].dwStartClusterIndex;
			...
			vcTmp[iIndexTmp] = vcPath[iIndex];
			iIndexTmp++;

		}
		iIndex++;
	}
	kMemCpy(pcDirName, vcTmp, iIndexTmp);
...
struct kDirectoryEntryStruct* kReadDirectory( DIR* pstDirectory )
{
	...
	while( pstDirectoryHandle->iCurrentOffset < FILESYSTEM_MAXDIRECTORYENTRYCOUNT )
	{
		...
		if( pstEntry[ pstDirectoryHandle->iCurrentOffset ].dwStartClusterIndex
			!= 0 || (pstEntry[ pstDirectoryHandle->iCurrentOffset ].bFileType == FILESYSTEM_TYPE_DIRECTORY && pstEntry[ pstDirectoryHandle->iCurrentOffset ].dwStartClusterIndex == 0))
		{
			// 동기화
			kUnlock( &( gs_stFileSystemManager.stMutex ) );
